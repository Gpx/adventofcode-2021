...
function fullyFoldMatrix(matrix, foldings) {
  return foldings.reduce((matrix, folding) => {
    const newMatrix = foldMatrix(matrix, folding);
    return newMatrix;
  }, matrix);
}
...
async function main() {
  ...
  matrix = fullyFoldMatrix(matrix, foldings);
  printMatrix(matrix);
