...
function calculateFirstSync(matrix, count = 1) {
  ...
  const octopi = newMatrix.reduce((count, line) => count + line.length, 0);
  if (allFlashes.length === octopi) return count;
  return calculateFirstSync(newMatrix, count + 1);
}
...
async function main() {
  ...
  printMatrix(matrix);
  const firstSync = calculateFirstSync(matrix);
